...
Also, the items in parenthesis are just to give you a better idea of what to write about, and should also be omitted from your response.
    ...
    logger.log("Generating Main Story Elements", 4)
    messages = [interface.build_user_query(prompt)]
    messages = interface.safe_generate_text(
        logger, messages, writer.config.INITIAL_OUTLINE_WRITER_MODEL, min_word_count=150
    )
    elements: str = interface.get_last_message_text(messages)
    logger.log("Done Generating Main Story Elements", 4)
